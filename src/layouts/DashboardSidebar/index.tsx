...
import { userSidebarMenu } from "@/src/constants/dashboard-sidebar";
import { Button } from "@/src/components/ui/button";
import { RootState, useAppDispatch, useAppSelector } from "@/src/redux/store";
import { setShow } from "@/src/redux/slices/dashboardSidebarSlice";
            ...
            item.label === "Notifications" ? (
              <li key={item.label} className="w-full">
                <Link
                  href={item.href}
                  onClick={() => dispatch(setShow(false))}
                  className={`flex justify-between items-center gap-2.5 py-2 px-1.5 rounded-[10px]  hover:text-white transition-all duration-200 font-sans  ${
                    pathname === item.href
