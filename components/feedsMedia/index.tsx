...
const FeedsMedia = ({ media = [], maxVisible = 4 }) => {
  ...
  return (
    <>
      <div
        className={`grid grid-cols-2 gap-2.5 ${getGridLayout(
          visibleMedia.length
        )}`}
      >
      ...
      {fullScreenMedia && <div className="fixed inset-0 z-50 bg-black bg-opacity-90 flex items-center justify-center p-4" onClick={closeFullScreen}>
          <div className="relative max-w-4xl max-h-full">
            ...
            {fullScreenMedia?.type === 'video' || fullScreenMedia?.url.match(/\.(mp4|webm|ogg)$/i) ? (
              <video
              width="320"
              height="240"
              controls
            ...
            >
              ...
              width={640}
              height={240}
              alt={fullScreenMedia?.alt || 'Fullscreen media'}
              className="max-w-full max-h-full rounded-lg"
              onClick={(e) => e.stopPropagation()}
            />
            )}
